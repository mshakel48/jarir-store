...
function AdminLayout() {
  ...
  const [email, setEmail] = useState("info@jarir.world");
    ...
    return (
      <div className="flex min-h-dvh items-center justify-center bg-background px-4">
        <form
          ...
          onSubmit={(e) => {
            e.preventDefault();
            if (!login(email, password) || useAuthStore.getState().users.find((u) => u.email === email.trim().toLowerCase())?.role !== "admin") {
              setError(t("auth.bad"));
              return;
            }
          }}
        >
          ...
          <p className="text-xs text-muted-foreground">info@jarir.world / admin123</p>
          <div>
